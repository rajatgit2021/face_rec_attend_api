...
def attendance(name):
    qry = f"SELECT * FROM emp_attendance where emp_id = '{name}'"
    print("qry :: "+qry)
    cursor.execute(qry)
    ...
    if len(records) == 0:
        print("No record with the emp id: "+name)
        ins = '''insert into emp_attendance (emp_id,attend_date,attend_time) values 
            (?,?,?)
            '''
        cursor.execute(ins,(name,dStr,tStr))
        conn.commit()
        #conn.close()
